...
module output_port_3by3 #(
    ...
    // --- OPTIMIZED COMBINATIONAL BLOCK ---
    // The original 25-step loop reduces mathematically to a single conditional addition/subtraction.
    reg [DATA_WIDTH-1:0] processed_comb;
    always @(*) begin
        if (selected_data[3:0] > 4'd7)
            processed_comb = selected_data - 5;
        else
            processed_comb = selected_data + 3;
    end
