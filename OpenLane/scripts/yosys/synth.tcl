...
proc run_strategy {output script strategy_name {postfix_with_strategy 0}} {
    ...
    if { [info exists ::env(SYNTH_AUTONAME)] && $::env(SYNTH_AUTONAME) } {
        # This native block converts $auto names to descriptive public names safely
        autoname
    }
    ...
    # Write the Verilog netlist for OpenROAD/OpenSTA
    write_verilog -noexpr -nohex -nodec -defparam $output
    
    # Write the JSON netlist into the same directory as the Verilog file 
    # so it isn't overwritten in multi-strategy exploratory runs.
    set json_output [string map {".nl.v" ".nl.json" ".v" ".json"} $output]
